#!/usr/bin/env python3
"""
Process NYC taxi trip data
Save to postgis database for future visualization.
#######
Credits:
pylint3 & black were used to check syntax and PEP8 compliance
"""
...
def create_holiday_df():
    ...
    for thisyear in range(2008, date.today().year + 2):
        for holiday_date, holiday_name in sorted(
            holidays.US(state="NY", years=thisyear).items()
        ):
...
def read_clean_csv(s3file):
    ...
    raw_df = spark.read.csv(
        s3file, header=True, inferSchema=True, multiLine=True, escape='"'
    )
    raw_df = removeColumnSpecialChar(raw_df)
    raw_df = (
        raw_df.withColumnRenamed("Trip_Pickup_DateTime", "pickup_datetime")
    ...
    raw_df = raw_df.select(
        ...
        round(
            (
                col("dropoff_datetime").cast(LongType())
                - col("pickup_datetime").cast(LongType())
            )
            / 3600,
            2,
        ).alias("duration"),
    ...
    raw_df = raw_df.filter(
        "pickup_year > 2007 and pickup_year < year(current_date()) + 1"
    )

    raw_df = raw_df.select(
        "*",
        round(col("trip_distance") / col("duration"), 2).alias("speed"),
        when((col("pickup_weekday") == 1) | (col("pickup_weekday") == 7), 1)
        .otherwise(0)
        .alias("isweekend"),
    )
    ...
    if "pickup_longitude" in raw_df.schema.names:
        raw_df = raw_df.filter(
            (raw_df.pickup_latitude != 0) & (raw_df.pickup_longitude != 0)
        )
        ##########################
        ...
        raw_df['PULocationID'] = [
            geo2taxizoneid(
                pickup_latitude,
                pickup_longitude
            ) for pickup_latitude, pickup_longitude in zip(
        ...
        raw_df['DOLocationID'] = [
            geo2taxizoneid(
                dropoff_latitude,
                dropoff_longitude
            ) for dropoff_latitude, dropoff_longitude in zip(
...
def process_tripdata(raw_df, sourceid):
    ...
    tripdata_df = raw_df.join(
        broadcast(holiday_df),
        (holiday_df.holiday_year == raw_df.pickup_year)
        & (holiday_df.holiday_month == raw_df.pickup_month)
        & (holiday_df.holiday_day == raw_df.pickup_day),
        how="left",
    ).drop("holiday_year", "holiday_month", "holiday_day")
    tripdata_df = (
        tripdata_df.withColumn(
            "pickup_month",
            when(tripdata_df["isholiday"] == 1, 0).otherwise(
                tripdata_df["pickup_month"]
            ),
        )
        .withColumn(
            "isweekend",
            when(tripdata_df["isholiday"] == 1, -1).otherwise(tripdata_df["isweekend"]),
        )
...
def geo2taxizoneid(lat, lng):
    ...
    """
    mysql = (
        "SELECT tz.locationid FROM taxi_zones tz WHERE st_within(ST_SetSRID( ST_Point( "
        + str(lng)
        + ","
        + str(lat)
        + "), 4326),tz.geom ) LIMIT 1"
    )
    zoneid = (
...
def save2db(df, table):
    ...
    df.write.format("jdbc").mode("append").option(
        "url", "jdbc:postgresql://postgis:5432/" + os.environ["PGDB"]
    ).option("dbtable", table).option("user", os.environ["PGUSER"]).option(
...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder.appName("smartrider").getOrCreate()
    ##############
